        ...
        <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="570px">
          <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="66px">
            ...
            <!-- <el-form-item label="状态" prop="enabled">
              {{typeof form.enabled }}{{dict.user_status}}
            </el-form-item> -->
            ...
            <el-form-item label="状态">
              <el-radio-group v-model="form.enabled" >
                <el-radio
                  v-for="(item, index) in dict.user_status"
                  :key="index"
                  :label="item.value"
                >{{ item.label }}</el-radio>
              </el-radio-group>
        ...
        <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
          ...
          <el-table-column :show-overflow-tooltip="true" prop="dept" label="部门">
            <template slot-scope="scope">
              <div>{{ deptsName(scope.row.depts) }}</div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    deptsName(depts) {
      let arr = depts.map(item=> item.name)
      return arr.join(',')
    ...
    [CRUD.HOOK.beforeToEdit](crud, form) {
      ...
      const _this = this
      // this.depts = form.depts
      form.depts.forEach(function(dept) {
        _this.deptDatas.push(dept.id)
      })
      console.log(_this.deptDatas, '_this.deptDatas',form.depts, this.depts)
    ...
    [CRUD.HOOK.afterValidateCU](crud) {
      ...
      const depts = []
        ...
        // this.$set(crud.form, 'depts', depts)
    ...
    synchAction() {
      ...
      crudUser.synch().then(res => {
          this.crud.refresh()
          loading.close();
          this.crud.notify('同步用户数据成功', CRUD.NOTIFICATION_TYPE.SUCCESS)
        }).catch(()=>{
          loading.close();
        })
    }
